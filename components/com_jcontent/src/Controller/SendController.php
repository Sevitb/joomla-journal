...
class SendController extends BaseFromController
{
        ...
        if (array_key_exists('files', $input->files->get('jform', array()))) {
            ...
            foreach ($files as $key => $file) {
                if ($file['error'] == 4) {
                    ...
                    $relativePathname = Path::clean('documents/materials/' . $userId . '/' . $lang->transliterate(str_replace(' ', '_', trim(htmlspecialchars($data['title'])))) . '/' . $lang->transliterate(str_replace(' ', '_', trim($file['name']))));
