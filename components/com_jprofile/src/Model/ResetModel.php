...
class ResetModel extends FormModel
{
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jprofile.reset_request', 'reset_request', array('control' => 'jform', 'load_data' => $loadData));
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jprofile.reset_complete', 'reset_complete', $options = array('control' => 'jform'));
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jprofile.reset_confirm', 'reset_confirm', $options = array('control' => 'jform'));
    ...
    protected function populateState()
    {
        ...
        $params = Factory::getApplication()->getParams('com_jprofile');
    ...
    public function processResetComplete($data)
    {
        ...
        $token = $app->getUserState('com_jprofile.reset.token', null);
        $userId = $app->getUserState('com_jprofile.reset.user', null);

        // Check the token and user id.
        if (empty($token) || empty($userId)) {
            return new \Exception(Text::_('COM_JPROFILE_RESET_COMPLETE_TOKENS_MISSING'), 403);
        }
        ...
        if (empty($user) || $user->activation !== $token) {
            $this->setError(Text::_('COM_JPROFILE_USER_NOT_FOUND'));
        ...
        if ($user->block) {
            $this->setError(Text::_('COM_JPROFILE_USER_BLOCKED'));
        ...
        if (!$user->save(true)) {
            return new \Exception(Text::sprintf('COM_JPROFILE_USER_SAVE_FAILED', $user->getError()), 500);
        }
        ...
        $app->setUserState('com_jprofile.reset.token', null);
        $app->setUserState('com_jprofile.reset.user', null);
    ...
    public function processResetConfirm($data)
    {
        ...
        try {
            ...
            return new \Exception(Text::sprintf('COM_JPROFILE_DATABASE_ERROR', $e->getMessage()), 500);
        }
        ...
        $app = Factory::getApplication();
        $app->setUserState('com_jprofile.reset.token', $user->activation);
        $app->setUserState('com_jprofile.reset.user', $user->id);
    ...
    public function processResetRequest($data)
    {
        ...
        $match = UserHelper::verifyPassword($data['password'], $currentUser->password, $currentUser->id);

        if ($match === false) {
            $this->setError(Text::_('JGLOBAL_AUTH_INVALID_PASS'));

            return false;
        }
        // Let's hash the entered password even if we don't have a matching user for some extra response time
        // By doing so, we mitigate side channel user enumeration attacks
        UserHelper::hashPassword($data['password']);
        ...
        if ($currentUser->authorise('core.admin')) {
            $this->setError(Text::_('COM_JPROFILE_REMIND_SUPERADMIN_ERROR'));

            return false;
        }

        // Make sure the user has not exceeded the reset limit
        if (!$this->checkResetLimit($currentUser)) {
            $resetLimit = (int) Factory::getApplication()->getParams()->get('reset_time');
            $this->setError(Text::plural('COM_JPROFILE_REMIND_LIMIT_ERROR_N_HOURS', $resetLimit));
        ...
        $currentUser->activation = $hashedToken;

        // Save the user to the database.
        if (!$currentUser->save(true)) {
            return new \Exception(Text::sprintf('COM_JPROFILE_USER_SAVE_FAILED', $currentUser->getError()), 500);
        }

        // Assemble the password reset confirmation link.
        $mode = $app->get('force_ssl', 0) == 2 ? 1 : (-1);
        $link = 'index.php?option=com_jprofile&view=reset&layout=confirm&token=' . $token;

        // Put together the email template data.
        $data = $currentUser->getProperties();
        $data['sitename'] = $app->get('sitename');
        $data['link_text'] = Route::_($link, false, $mode);
        $data['link_html'] = Route::_($link, true, $mode);
        $data['token'] = $token;

        $mailer = new MailTemplate('com_jprofile.password_reset', $app->getLanguage()->getTag());
        $mailer->addTemplateData($data);
        $mailer->addRecipient($currentUser->email, $currentUser->name);
        ...
        // Check for an error.
        if ($return !== true) {
            return new \Exception(Text::_('COM_JPROFILE_MAIL_FAILED'), 500);
        } else {
            return true;
        }
    }
